...
定义空基类 `AllocatorPolicyBase`，所有分配器类（`LinearAllocator`、`HeapAllocator`、`LinearAllocatorWithFallback`、`PoolAllocator`、`PoolAllocatorWithFallback`）公有继承之。`Arena` 模板在实例化时用 `static_assert(std::is_base_of_v<AllocatorPolicyBase, AllocatorPolicy>)` 验证传入的分配器类型，验证失败产生编译错误。基类不含虚函数（Arena 按值持有分配器且模板编译期分派，虚函数无意义）。

#### Scenario: 合法分配器

- **WHEN** 以 `LinearAllocator`、`HeapAllocator` 等内置分配器实例化 `Arena`
- **THEN** 编译通过（`static_assert` 满足）

#### Scenario: 非法分配器

- **WHEN** 以不继承 `AllocatorPolicyBase` 的类型实例化 `Arena`
- **THEN** 编译期报错，错误信息指明必须是 `AllocatorPolicyBase` 的派生类
...
`HeapAllocator` 每次 `Alloc(size, alignment)` 走对齐堆分配（`posix_memalign`/Windows `_aligned_malloc`），`Free(p)` 对应释放；`Swap` 为空操作。分配器无状态、可默认构造。
...
定义空基类 `AreaPolicyBase`（不含虚函数），**不再存在 `AreaPolicy` 命名空间**。三个 Area 类继承之并上提为平级类：
...
`LockingPolicy` 命名空间提供 `NoLock`（lock/unlock 空操作）与 `Mutex`（`using Mutex = std::mutex`）两种锁策略，供 `Arena` 模板使用。`NoLock` 为进程内单线程场景零开销；`Mutex` 配合 `std::lock_guard` 提供互斥。
...
- 模板参数经 `static_assert` 验证为 `AllocatorPolicyBase` / `AreaPolicyBase` 派生类
...
### Requirement: TrackingPolicy 跟踪策略

**Reason**: 按移植需求，Arena 去除 TrackingPolicy 功能——删除 `Untracked`/`HighWatermark`/`Debug`/`DebugAndHighWatermark` 及 Arena 上全部 listener 接口，连带删除 name 参数。

**Migration**: 无历史使用者（全新移植）；如需跟踪能力，后续可基于 Arena 外部包装或回调另行设计。
