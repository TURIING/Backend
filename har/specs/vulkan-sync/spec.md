# Capability: vulkan-sync

## Purpose

提交同步原语：`VkFence` 的状态包装与池化（`VulkanCmdFence` / `VulkanFencePool`）、提交信号量的 Resource 包装与池化（`VulkanSemaphore` / `VulkanSemaphoreManager`），以及跨文件共享常量与 `FenceStatus` 通用枚举。围栏句柄经回收回调归还其池，信号量句柄在资源析构时归还——常规路径上都不销毁句柄。
...
SHALL 提供静态 `Completed()`：返回状态为 `VK_SUCCESS`、持有 `VK_NULL_HANDLE` 的实例（未提交任何命令即视为全部完成）。`GetVkFence()` SHALL 返回底层句柄。析构 SHALL 在回收回调非空时以 `m_fence` 调用它。类 SHALL 位于 `src/vulkan/sync/VulkanCmdFence.h/.cpp`，由 `VulkanFencePool` 提供回收回调。
...
`VulkanFencePool` SHALL 提供构造 `(const VulkanContextPtr&, VkDevice, uint32_t minPoolSize)`，构造时预分配 `minPoolSize` 个 `VkFence`（分配失败者不计入 `m_numFences`）。`AcquireFenceStatus()` SHALL 取一个空闲围栏并返回绑定回收回调（回收到本池）的 `shared_ptr<VulkanCmdFence>`，同时把它的 `weak_ptr` 登记进跟踪表。
...
`VulkanSemaphore` SHALL public 继承 `Resource`，持有 `VulkanSemaphoreManagerPtr` 与 `VkSemaphore`，经 `ResourceManager::AllocateAndConstruct<VulkanSemaphore>(VulkanSemaphoreManagerPtr(this), semaphore)` 构造（不接受裸 `new`）；SHALL 提供 `GetVkSemaphore()` 只读访问。析构 SHALL 调用 `m_manager->Recycle(m_semaphore)` 把句柄归还信号量池。
