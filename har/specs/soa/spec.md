...
SoA（Structure of Arrays）容器：`SoaBase<Allocator, Elements...>` 以单次分配的连续缓冲存储多列，列间按类型对齐补齐；`Soa<E...>` 别名默认走 `HeapAllocator`。支持按列访问（Data/Begin/End/Slice/ElementAt/Back）、按行访问（`operator[]` 行引用代理、`std::span` 切片）、随机访问迭代器（STL 算法整行搬运）与 `Field` 字段引用，`CopyRange` 支持跨分配器拷贝；拷贝禁用、移动转移所有权。移植自 Filament StructureOfArrays，命名压缩为 Soa。
