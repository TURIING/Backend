...
可切分的共享 CPU→GPU 暂存缓冲：`VulkanStageBuffer` 提供可持久映射的暂存内存，其嵌套资源类 `Segment` 表达切分单元并随引用归零经 `vulkan-resource` 通道回收；`VulkanStagePool` 按剩余容量复用缓冲、跨帧淘汰已无 Segment 占用的缓冲。当前为纯 buffer 形态——`VulkanStageImage` 与图像淘汰路径未移植。
