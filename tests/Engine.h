...
public:
    ...
    Backend::SwapChainHandle CreateSwapChain(void* nativeWindow, uint64_t flags);
    ...
    void ReadPixels(Backend::RenderTargetHandle src, uint32_t x, uint32_t y, uint32_t width, uint32_t height, Backend::PixelBufferDescriptor&& pbd);
    void WaitForReadPixels();

    // 把 finish() 投到渲染线程执行并等待其返回：读回请求的登记与投递都发生在该线程上
    bool FlushAndFinish();

    NODISCARD Backend::PlatformPtr GetPlatform() const noexcept;
...
private:
    ...
    Backend::CommandBufferQueuePtr m_queue;
